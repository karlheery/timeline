...
class PolaroidDisplay extends _react.Component {
  constructor(props) {
    super(props);
  ...
  showMedia(self) {
    ...
    if (!indexArray || indexArray.length <= 0) {
      ...
      indexArray = Array.apply(null, {
        length: self.state.medialist.length
    ...
    self.setState({
      quadrant: self.state.quadrant + 1
    });
    ...
    if (Math.floor(Math.random() * 2 + 1) == 2) rotationAngle = rotationAngle * -1;
    ...
    */
    ...
    this.fetchFile(mediafile).then(function (result) {
      ...
      var dateTaken;
      var comment;
      _exifJs.default.getData(imageFile, function () {
        var orientation = _exifJs.default.getTag(this, "Orientation");
        var dateTaken = _exifJs.default.getTag(this, "DateTimeOriginal");
  ...
  */
  saveImagePosition(mediaFile, finalXPos, finalYPos, scaledWidth, scaledHeight, cmt) {
    ...
    this.state.imageLocations.push({
      imageDisplayed: mediaFile,
      X: finalXPos,
      Y: finalYPos,
      width: scaledWidth,
      height: scaledHeight,
  ...
  getImageAtPosition(X, Y) {
    for (var i = 0; i < this.state.imageLocations.length; i++) {
      ...
      //
      if (canvasImage.X - canvasImage.width / 2 < X && canvasImage.X + canvasImage.width / 2 > X && canvasImage.Y - canvasImage.height / 2 < Y && canvasImage.Y + canvasImage.height / 2 > Y) {
